...
export const startGame = async (req: Request, res: Response) => {
  try {
    ...
    console.log('[DRAGON-TOWER-CONTROLLER] Starting game:', {
      userId,
      betAmount,
    ...
    const gameId = `dragontower_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    
    const game = await CasinoGame.create({
      gameId,
      userId,
      gameType: 'dragontower',
      betAmount,
      status: 'active',
      createdAt: new Date()
    });
    ...
    console.log('[DRAGON-TOWER-CONTROLLER] Game started:', {
      gameId,
    ...
    console.error('[DRAGON-TOWER-CONTROLLER] Error starting game:', error);
    res.status(500).json({ 
      success: false,
...
export const processWin = async (req: Request, res: Response) => {
  try {
    ...
    console.log('[DRAGON-TOWER-CONTROLLER] Win processed:', {
      gameId,
    ...
    console.error('[DRAGON-TOWER-CONTROLLER] Error processing win:', error);
    res.status(500).json({ 
      success: false,
...
// Add to dragonTowerController.ts
export const processLoss = async (req: Request, res: Response) => {
    try {
      ...
      console.error('[DRAGON-TOWER-CONTROLLER] Error processing loss:', error);
      res.status(500).json({ 
        success: false,
        message: 'Error processing loss' 
      });
    }
  };